...
function inventoryMatch(name,list){const cs=candidates(name);return list.find(x=>{const v=norm(x.text);return cs.some(c=>v===c||(c.length>=4&&(v.includes(c)||c.includes(v))))})}
...
function renderNice(){
 ...
 const shop=nice.shopping.map(x=>`<div class="nice-shop-card"><button data-nice-bought="${x.id}" class="nice-ok">✓</button><span>${esc(x.text)}</span><button data-nice-delshop="${x.id}" class="nice-x">×</button></div>`).join('')||'<div class="nice-empty-state"><span>✓</span><strong>Compra al día</strong><p>No tienes nada pendiente.</p></div>';
